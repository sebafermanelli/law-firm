...
import com.solvd.custom.MyRunnable;
import com.solvd.custom.MyThread;
import com.solvd.enums.*;
import com.solvd.exceptions.*;
import com.solvd.custom.MyLinkedList;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        LegalAssistant assistant1 = new LegalAssistant("Jackson Villa", 3, LegalSpecialization.FAMILY_LAW);
        LegalAssistant assistant2 = new LegalAssistant("Ethan Walker", 6, LegalSpecialization.CIVIL_COMMERCIAL_LAW);
        LegalAssistant assistant3 = new LegalAssistant("Emma Wallace", 8, LegalSpecialization.LABOR_LAW);
        Lawyer lawyer1 = new Lawyer("Anna Scott", 3, LegalSpecialization.FAMILY_LAW, "LN-29184");
        Lawyer lawyer2 = new Lawyer("Liam Anderson", 7, LegalSpecialization.CIVIL_COMMERCIAL_LAW, "LN" + "-92834");
        ...
        } catch (LegalSecretaryExistException | LegalAssistantExistException | LawyerExistException | ClientExistException e) {
...
//      FileUtils.writeStringToFile(file, "Amount of unique words in the file: " + uniqueWords.size(), "UTF-8", false);
        ...
        BiPredicate<Lawyer, Lawyer> lawyerSpecializationBiPredicate = (l1, l2) -> l1.getSpecialization().equals(l2.getSpecialization());
        LOGGER.info(lawyer4.getName() + " and " + lawyer2.getName() + " has the same specialization? " + lawyerSpecializationBiPredicate.test(lawyer4, lawyer2));

        //Collections streaming
        try {
            Set<String> lawyers5YearExperience = lawFirm.getLawyers().stream().filter(lawyer -> lawyer.getExperienceYears() > 5).map(lawyer -> lawyer.getName()).collect(Collectors.toSet());
        ...
        try {
            Set<LegalCase> openCases = lawFirm.getCases().stream().filter(c -> c.getStatus().equals(CaseStatus.OPEN)).collect(Collectors.toSet());
            openCases.forEach(c -> {
        ...
        try {
            Set<String> laborLawAssistants = lawFirm.getAssistants().stream().filter(assistant -> assistant.getSpecialization().equals(LegalSpecialization.LABOR_LAW)).map(assistant -> assistant.getName()).collect(Collectors.toSet());
        } catch (LegalAssistantListEmptyException e) {
            LOGGER.error(e.getMessage());
        }

        try {
            Set<String> clientsFromNY = lawFirm.getClients().stream().filter(client -> client.getContactInfo().contains("NY")).map(client -> client.getName()).collect(Collectors.toSet());
        } catch (ClientListEmptyException e) {
            LOGGER.error(e.getMessage());
        }

        try {
            Set<LegalDocument> draftDocumentsCase1 = legalCase.getDocuments().stream().filter(document -> document.getDocumentStatus().equals(DocumentStatus.DRAFT)).collect(Collectors.toSet());
        } catch (LegalDocumentListEmptyException e) {
            LOGGER.error(e.getMessage());
        }

        try {
            Set<LegalDocument> allApprovedDocuments = lawFirm.getCases().stream().flatMap(c -> {
                try {
                    return c.getDocuments().stream().filter(document -> document.getDocumentStatus().equals(DocumentStatus.APPROVED));
                } catch (LegalDocumentListEmptyException e) {
                    LOGGER.error(e.getMessage());
                    return Stream.empty();
                }
        ...
        for (Field field : courtClass.getDeclaredFields()) {
            LOGGER.info("Field{" + "name='" + field.getName() + "', " + "type='" + field.getType() + "', " + "modifiers='" + Modifier.toString(field.getModifiers()) + "', ");
        }

        LOGGER.info("Court class constructors information: ");
        for (Constructor constructor : courtClass.getDeclaredConstructors()) {
            LOGGER.info("Constructor{" + "name='" + constructor.getName() + "', " + "parameters='" + Arrays.toString(constructor.getParameters()) + "', " + "modifiers='" + Modifier.toString(constructor.getModifiers()) + "', ");
        }

        LOGGER.info("Court class methods information: ");
        for (Method method : courtClass.getDeclaredMethods()) {
            LOGGER.info("Method{" + "name='" + method.getName() + "', " + "return='" + method.getReturnType() + "', " + "parameters='" + Arrays.toString(method.getParameters()) + "', " + "modifiers='" + Modifier.toString(method.getModifiers()) + "', ");
        }
        ...
        Thread thread7 = new Thread(new MyRunnable());
        ...
        //Threads with CompletableFuture
        CompletableFuture<Void> completableFuture1 = CompletableFuture.runAsync(thread1);
        CompletableFuture<Void> completableFuture2 = CompletableFuture.runAsync(thread2);
        CompletableFuture<Void> completableFuture3 = CompletableFuture.runAsync(thread3);
        CompletableFuture<Void> completableFuture4 = CompletableFuture.runAsync(thread4);
        CompletableFuture<Void> completableFuture5 = CompletableFuture.runAsync(thread5);
        CompletableFuture<Void> completableFuture6 = CompletableFuture.runAsync(thread6);
        CompletableFuture<Void> completableFuture7 = CompletableFuture.runAsync(thread7);
